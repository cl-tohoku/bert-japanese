...
from tokenization import JapaneseBertTokenizer
...
class LdccProcessor(DataProcessor):
    """Processor for the Livedoor News Corpus data set."""

    def get_train_examples(self, data_dir):
        """See base class."""
        examples = []
        category_names = self.get_labels()
        for cat in category_names:
            filenames = glob.glob(os.path.join(data_dir, "text", cat, f"{cat}*.txt"))
            for fname in sorted(filenames):
                if int(fname[-5]) % 5 in (0, 1, 2):
                    examples.extend(self._create_examples(fname, cat, 'train'))
    ...
    def get_labels(self):
        """See base class."""
        return ["dokujo-tsushin", "it-life-hack", "kaden-channel", "livedoor-homme",
                "movie-enter", "peachy", "smax", "sports-watch", "topic-news"]

    def _create_examples(self, filename, category, set_type):
        """Creates examples for the training and dev sets."""
        examples = []
        with open(filename) as f:
            lines = f.readlines()[2:]  # 0: URL, 1: timestamp
            lines = [line.strip() for line in lines if line.strip()]
            text = ' '.join(lines)
            examples.append(InputExample(guid=filename, text_a=text, label=category))
...
def multiclass_pr_and_f1(preds, labels, label_list):
    ...
    for i, label in enumerate(label_list):
        results["Label {} ({})".format(i, label)] = \
            "Precision: {:.4f} ".format(precisions[i]) + \
            "Recall: {:.4f} ".format(recalls[i]) + \
            "F1 score: {:.4f} ".format(f1_scores[i]) + \
            "Support: {:>4d}".format(supports[i])
...
def compute_metrics(task_name, preds, labels, label_list=None):
    assert len(preds) == len(labels)
    if task_name == "ldcc":
        return multiclass_pr_and_f1(preds, labels, label_list)
    else:
...
def main():
    ...
    parser.add_argument("--bert_model", default=None, type=str, required=True,
                        help="Bert pre-trained model. It must be converted to the format of pytorch-pretrained-BERT.")
    ...
    parser.add_argument("--vocab_file",
                        ...
                        type=str,
                        required=True,
                        help="Path to a one-wordpiece-per-line vocabulary file.")
    ...
    parser.add_argument("--vocab_type",
                        default="bpe",
                        type=str,
                        help='Subword vocabulary type ("bpe" or "char").')
    parser.add_argument("--mecab_dict",
                        default=None,
                        type=str,
    ...
    tokenizer = JapaneseBertTokenizer(args.vocab_file, args.vocab_type,
                                      args.do_lower_case, args.mecab_dict)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        result = compute_metrics(task_name, preds, all_label_ids.numpy(), label_list)
